...
module Avatar
  ...
  included do
    has_attached_file :avatar,
                      ...
                      path: "#{self.to_s.downcase.pluralize}/:attachment/:id/:style/#{self.to_s.downcase}_brand",
                      url: ':s3_domain_url',
                      styles: { original: '300x300>', thumb: '75x75>' },
                      default_url: 'defaults/company.jpg', if: :avatar?
  ...
  def picture(type=:thumb)
    ...
    return self.company.avatar.url(type) if self.is_a?(App) && self.company.avatar_file_name
    nil
  end
